...
@Service
@RequiredArgsConstructor // (=) @Autowired 는 class 안에 변수마다 하나씩 설정
						 //@RequiredArgsConstructor 는 클래스 밖에서 전체적으로 설정해줌 (이름이 구체적..)
public class EmailService {

	//자바 자체에서 제공하는 이메일 보내기 기능 틀
	//왜 틀만 제공하냐? -> 이메일 전송하는데 어느회사 어느규격 이메일인지 알 수 없기 때문에
	//이메일을 보낼 때 보내는 이 | 내용 | 받는 이
	private final JavaMailSender javaMailSender;
	
	//이메일을 보낼 이메일 주소값
	private static final String senderEmail = "pubao020729@gmail.com"; //이메일을 보낼 담당자 이메일 작성
	...
	/* 
 	 ...
 	 001이 1로 변하는 magic
 	  └ 마치, 01055556666 이 1055556666 이 되는 것 처럼 ..
