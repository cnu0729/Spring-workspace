...
public class ParameterController {
	...
	/*
	 ...
	 - 매개 변수 앞에 해당 어노테이션을 작성하면, 매개변수에 값이 작성된
	 - 작성되는 데이터는 매개변수(파라미터) 타입에 맞게 형변환(parse) 자동으로 수행
	 
	 [속성 추가 작성법]
	 @RequestParam(value="name", required="false", defaultVale="1")
	 
	 value = 전달받은 input 태그의 name 속성ㄱ밧
	...
	*/
	...
	public String paramTest2(/*requestParam 여기에 보통 작성*/
			@RequestParam(/*value=*/"title"/*, required=true*/) String title,
			@RequestParam(/*value=*/"writer"/*, required=true*/) String writer,
			@RequestParam(/*value=*/"price"/*, required=true*/) int price,
			@RequestParam(value="publisher", defaultValue="교보문고") String publisher
			) {
	...
	public String paramTest3(@RequestParam(value="color", required=false) String[] colorArr,
		...
		// 같은 name 속성 파라미터가 String이나 List로 저장이 되는 것은 힘듬
	 ...
	 *DTO와 VO (주로 DTO 사용)
	 	└ DTO : Data Transfer Object 데이터 캡슐화를 통해 데이터를 전달하고 관리
	 			...
	 			계층이란 ? html 에서 db로 간다. (한 계층에서 다른 계층으로 전송)
	...
	*/
	/*
	 ...
	 [주의사항]
	 	...
	 	@ModelAttribute 이용해 값이 필드에 저정된 객체를 커맨드 객체라고 함
	*/
