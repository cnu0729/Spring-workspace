...
public class PigService {
	...
	public void uploadPig(String pig_name, int pig_age, MultipartFile file) {
		...
		//String uploadDir = "C:/Users/user1/Desktop/pig_image/";
		String uploadDir = "C:/Users/user1/servlet_jsp.workspace/demoProject3-mybatis-mapper-select-image/src/main/resources/static/images/";
		//맨끝에 /로 닫아주지 않으면 static/images 안에 이미지가 저장되는 것이 아니라
		//static 바로 밑에 생성되기 때문에 이미지가 보이지 않으므로 반드시 마지막에 /
		File imgfile = new File(uploadDir + fileName);
		// 나중에 이미지 파일을 저장하는 폴더가 존재하지 않ㅇ르 경우 생성하는 코드
		// 우리가 저장하고자 하는 파일 경로 설정
	...
	//나중에 이미지 업로드에서 파ㅏ일 저장 경로에 대한 함수만 따로 설정
